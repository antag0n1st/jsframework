(function (window, undefined) {
    ...
    Input.prototype.process_click = function () {
        ...
        event.point.x = this.point.x;
        event.point.y = this.point.y;

        if (type === 0) { // mouse up
            
            var is_propagation_stopped = false;

            for (var i = count; i--; ) {

                var element = elements[i];
                if (element && element.check(circlePoint)) {
                ...
                } else if(element && element.is_mouse_down) {
                    // this is not good for multitouch
                    this.process_cancel();
                }

            }

        } else if (type === 1) { // mouse down
    ...
    Input.prototype.process_move = function () {
        ...
        for (var i = count; i--; ) {
            var element = elements[i];
            if (element && element.is_mouse_down) {
