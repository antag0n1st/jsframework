...
Input.prototype.add_listener = function (element_id) {
    ...
    if (Config.is_mobile()) {
        ...
        window.addEventListener("touchcancel", function (evt) {
            for (var i = 0; i < evt.changedTouches.length; i++) {
                that.event_idx = evt.changedTouches[i].identifier;
                that.set_mouse_cancel(evt.changedTouches[i].pageX, evt.changedTouches[i].pageY, i);
            }
            evt.preventDefault();
            evt.stopPropagation();
        }, false);
...
Input.prototype.process_click = function () {
    ...
    if (type === 0) {
        ...
        for (var i = count; i--; ) {
            ...
            if (element && element.event_idx === this.event_idx && element.check(circlePoint)) {
        ...
        for (var i = count; i--; ) {
            ...
            if (element && element.check(circlePoint) && element.event_idx !== this.event_idx) {
...
Input.prototype.process_move = function () {
    ...
    for (var i = count; i--; ) {
        var element = elements[i];
        if (element && element.is_mouse_down && element.event_idx === this.event_idx) {
