(function (window) {
    ...
    AtlasAnimation.TYPE = UID.numbering();
...
//    ]
        ...
        this.texture = PIXI.Texture.fromFrame(Object.keys(confs[0].animations)[0] + '0');
    ...
    AtlasAnimation.prototype.play = function (anime_name, is_flipped, is_loop, callback) {
        ...
        var ca = this.current_animation;
        var f = this.texture.frame;

        this.set_size(ca.w * f.width, ca.h * f.height);
        this.set_anchor(ca.ax, ca.ay);
    ...
    AtlasAnimation.prototype.update = function (dt) {
        ...
        if (!this.is_paused && this.current_animation && !this.has_ended) {
            ...
            if (this.has_ended && this.callback && !this.is_callback_called) {
                this.is_callback_called = true;
                this.callback(this);
            }
        }
